...
--# FUNCTIONS
	...
	gal.mapgen.get_biome_name = function(heat,humid,y)
	...
	gal.mapgen.get_heat_scalar = function(z)
--[[
		if gal.mapgen.use_heat_scalar == true then
			...
			local t_heat_mid = ((gal.mapgen.mg_map_size * gal.mapgen.mg_world_scale) * 0.275)
			...
			local t_map_scale = t_heat_scale / gal.mapgen.mg_world_scale
...
--
	...
	gal.mapgen.get_humid_scalar = function(z)

		if gal.mapgen.use_humid_scalar == true then

			local t_z = abs(z)
			--local t_humid_mid = ((gal.mapgen.mg_map_size * gal.mapgen.mg_world_scale) * (1/12))
			local t_humid_mid = ((gal.mapgen.mg_map_size * gal.mapgen.mg_world_scale) * 0.062)
			local t_world = 0.0125 / gal.mapgen.mg_world_scale
			local t_diff = 0
