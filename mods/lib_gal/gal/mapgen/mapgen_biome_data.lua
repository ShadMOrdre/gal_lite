...
--[[
	...
	--gal.mapgen.heat_biome = {
	--	
	--}
	--gal.mapgen.humid_biome = {
	--	
	--}
	gal.mapgen.biomes = {}
	--for iheat in -37,137 do
	for iheat in 1,174 do

		--gal.mapgen.biomes.[tonumber(iheat)] = {}

		--for ihumid in -37,137 do
		for ihumid in 1,174 do
			local i_hh = {
				heat = tonumber(iheat),
				humid = tonumber(ihumid)
			}
			gal.mapgen.biomes.[i_hh] = {}
			--gal.mapgen.biomes.[tonumber(iheat)].[tonumber(ihumid)] = {}

			local t_heat, t_humid

			if iheat < m_top1 then
				t_heat = "cold"
			elseif iheat >= m_top1 and iheat < m_top2 then
				t_heat = "cool"
			elseif iheat >= m_top2 and iheat < m_top3 then
				t_heat = "temperate"
			elseif iheat >= m_top3 and iheat < m_top4 then
				t_heat = "warm"
			elseif iheat >= m_top4 then
				t_heat = "hot"
			end

			if ihumid < m_top1 then
				t_humid = "_arid"
			elseif ihumid >= m_top1 and ihumid < m_top2 then
				t_humid = "_semiarid"
			elseif ihumid >= m_top2 and ihumid < m_top3 then
				t_humid = "_temperate"
			elseif ihumid >= m_top3 and ihumid < m_top4 then
				t_humid = "_semihumid"
			elseif ihumid >= m_top4 then
				t_humid = "_humid"
			end

			gal.mapgen.biomes.[i_hh] = t_heat .. t_humid
			--gal.mapgen.biomes.[tonumber(iheat)].[tonumber(ihumid)] = t_heat .. t_humid
		end
	end
--]]
