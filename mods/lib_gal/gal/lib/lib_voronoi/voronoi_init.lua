...
--##
	...
	function gal.lib.voronoi.get_nearest_cell(ppoints, pos, dist_type, tier)
		...
		--local t_points = gal.lib.voronoi.points[ppoints]
		local t_points = ppoints
	
		local closest_cell_idx = 0
		local closest_cell_x
		local closest_cell_z
		...
		for i, point in ipairs(t_points) do
			...
			if point.tier == tier then
				...
				if last_dist then
					if last_dist > this_dist then
						...
						closest_cell_x = point.x
						closest_cell_z = point.z
						last_dist = this_dist
						last_closest_idx = i
...
--]]
		...
		return closest_cell_idx, closest_cell_dist, closest_cell_z, closest_cell_x, t_z, t_x, edge
	
	end
	
	function gal.lib.voronoi.get_closest_cell_from_neighbors(ppoints, pos, dist_type, cell_idx)
		...
		local c_x = pos.x - t_points[cell_idx].x
		local c_z = pos.z - t_points[cell_idx].z
		local c_dist = gal.lib.metrics.get_dist(c_x, c_z, d_type)

		for i, neighbor in pairs(gal.lib.voronoi.neighbors[cell_idx]) do

			t_x = pos.x-t_points[i].x
			t_z = pos.z-t_points[i].z
...
--##
	...
	function gal.lib.voronoi.get_cell_neighbors(ppoints, cell_idx)
			...
			for i, i_point in ipairs(t_points) do
				...
				if i_point.tier == curr_cell.tier then
					...
					if i ~= cell_idx then
						...
						t_mid_cell = gal.lib.voronoi.get_nearest_cell(t_points, {x = t_mid_x, z = t_mid_z}, dist_metric, curr_cell.tier)
	...
	function gal.lib.voronoi.get_all_neighbors(ppoints)

		for i_p, i_point in ipairs(gal.lib.voronoi.points[ppoints]) do
...
function gal.lib.voronoi.get_vertices_data()
	...
	print("[gal.lib.voronoi] Voronoi Vertices Data Total Time " .. chugent .. " seconds")
...
function gal.lib.voronoi.load_vertices()
	...
	if (t_vertices == nil) then
		...
		minetest.log("[gal.lib.voronoi] Voronoi Vertex Data loaded from file.")
...
--##
	...
	function gal.lib.voronoi.make_voronoi_recursive_lite(pname, cells_x, cells_y, cells_z, size, dist_type)
		...
		local t0 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Generation Start")
		...
		gal.lib.voronoi.points[pname] = {}

		local v_idx = 1
	
		for i_c = 1, cells_x do
	
			--local m_pt_x = t_points[name][v_idx].x
			--local m_pt_z = t_points[name][v_idx].z
			...
			gal.lib.voronoi.points[pname][v_idx] = {}
			gal.lib.voronoi.points[pname][v_idx].z = m_pt_z
			gal.lib.voronoi.points[pname][v_idx].x = m_pt_x
	
			gal.lib.voronoi.points[pname][v_idx].tier = 1
	
			--t_points[v_idx] = {x = m_pt_x, z = m_pt_z}
			temp_points = temp_points .. v_idx .. "|" .. m_pt_z .. "|" .. m_pt_x .. "|" .. "1" .. "\n"
		...
		local t1 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Tier 1 Generation Time: " .. (t1-t0) .. " ms")
		...
		for i_t = 1, (cells_x * cells_y) do
			...
			gal.lib.voronoi.points[pname][v_idx] = {}
			gal.lib.voronoi.points[pname][v_idx].z = p_pt_z
			gal.lib.voronoi.points[pname][v_idx].x = p_pt_x
	
			gal.lib.voronoi.points[pname][v_idx].tier = 2
	
			temp_points = temp_points .. v_idx .. "|" .. p_pt_z .. "|" .. p_pt_x .. "|" .. "2" .. "\n"
	
			v_idx = v_idx + 1
		end
		temp_points = temp_points .. "#" .. "\n"
		local t2 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Tier 2 Generation Time: " .. (t2-t1) .. " ms")
		...
		for i_c = 1, (cells_x * cells_y * cells_z) do
			...
			gal.lib.voronoi.points[pname][v_idx] = {}
			gal.lib.voronoi.points[pname][v_idx].z = c_pt_z
			gal.lib.voronoi.points[pname][v_idx].x = c_pt_x
	
			gal.lib.voronoi.points[pname][v_idx].tier = 3
	
			temp_points = temp_points .. v_idx .. "|" .. c_pt_z .. "|" .. c_pt_x .. "|" .. "3" .. "\n"
		...
		temp_points = temp_points .. "#" .. "\n"
		local t3 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Tier 3 Generation Time: " .. (t3-t2) .. " ms")
	
		--gal.lib.csv.save(temp_points, gal.path_world .. "/" .. gal.lib.voronoi.file2d)
		gal.lib.csv.save_worldpath(temp_points, pname)
		--gal.lib.voronoi.save_points(pname)

		local t4 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Save Data Time: " .. (t4-t3) .. " ms")
	
		-- Random Points generation finished. Check the timer to know the elapsed time.
		local t5 = os.clock()
		minetest.log("[gal.lib.voronoi] Recursive Voronoi Cell Random Points Total Generation Time: " .. (t5-t0) .. " ms")
...
--## 2D
	function gal.lib.voronoi.load_points_lite(pfile, pscale)
		...
		if (t_points == nil) then
			t_points = gal.lib.csv.load_worldpath("|", pfile)
		end
	
		if (t_points == nil) then
	
			minetest.log("[gal.lib.voronoi] Voronoi Cell Points file not found.  Using randomly generated points.")
			print("[gal.lib.voronoi] Voronoi Cell Points file not found.  Using randomly generated points.")
	
			gal.lib.voronoi.make_voronoi_recursive_lite(pfile, voronoi_recursion_1, voronoi_recursion_2, voronoi_recursion_3, map_size, dist_metric)
			--gal.lib.voronoi.save_points(pfile)
		else
	
			gal.lib.voronoi.points[pfile] = {}

			for i_p, p_point in ipairs(t_points) do
		
				local idx, p_z, p_x, p_tier = unpack(p_point)

				--if not (gal.lib.voronoi.points[pfile]) then
				--	gal.lib.voronoi.points[pfile] = {}
				--end

				--tonumber(idx)
				gal.lib.voronoi.points[pfile][tonumber(idx)] = {}
		
				gal.lib.voronoi.points[pfile][tonumber(idx)].z = (tonumber(p_z)) * pscale
				--gal.lib.voronoi.points[pfile][tonumber(idx)].z = tonumber(p_z)
				gal.lib.voronoi.points[pfile][tonumber(idx)].x = (tonumber(p_x)) * pscale
				--gal.lib.voronoi.points[pfile][tonumber(idx)].x = tonumber(p_x)

				gal.lib.voronoi.points[pfile][tonumber(idx)].tier = tonumber(p_tier)
		
			end
	
			minetest.log("[gal.lib.voronoi] Voronoi Cell Points loaded from file " .. pfile .. ".")
	
		end
	end

--## 3D
	function gal.lib.voronoi.load_points_3d_lite(pfile, pscale)
		...
		if (t_points == nil) then
			...
			for i_p, p_point in ipairs(t_points) do
				...
				local idx, p_z, p_y, p_x, p_tier = unpack(t_points)
	
				gal.lib.voronoi.points[tonumber(idx)] = {}
		
				gal.lib.voronoi.points[tonumber(idx)].z = tonumber(p_z) * pscale
				gal.lib.voronoi.points[tonumber(idx)].y = tonumber(p_y) * pscale
				gal.lib.voronoi.points[tonumber(idx)].x = tonumber(p_x) * pscale
	...
	function gal.lib.voronoi.save_points(pfile)
		...
		for i_c, i_cell in pairs(gal.lib.voronoi.points[pfile]) do

			local temp_points = "#C_Idx|C_Z|C_X|Tier\n"
	

			--minetest.log("i_cell: " .. dump(i_cell))
			--print("i_cell: " .. dump(i_cell))
	
			temp_points = temp_points .. i_c .. "|" .. i_cell.z .. "|" .. i_cell.x .. "|" .. i_cell.tier .. "\n"

			temp_points = temp_points .. "#" .. "\n"

		end

		gal.lib.csv.save_worldpath(temp_points, pfile)
		--gal.lib.csv.save(temp_points, pfile)
...
--##
	...
	function gal.lib.voronoi.load_neighbors(pfile)
		...
		if (t_neighbors == nil) then
			t_neighbors = gal.lib.csv.load_worldpath("|", pfile)
		end
		...
		if (t_neighbors == nil) then
			...
			minetest.log("[gal.lib.voronoi] Voronoi Cell Neighbors loaded from file.")
	...
	function gal.lib.voronoi.save_neighbors(pfile)
		...
		--gal.lib.csv.save(temp_neighbors, pfile)
	end
...
--##
	...
	function gal.lib.voronoi.load_edgemap(pfile)
		...
		if (t_edgemap == nil) then
			t_edgemap = gal.lib.csv.load_worldpath("|", pfile)
		end
		...
		if (t_edgemap ~= nil) then
			...
			minetest.log("[gal.lib.voronoi] Voronoi Edge Map loaded from file.")
	...
	function gal.lib.voronoi.save_edgemap(pfile)
		...
		gal.lib.csv.save_worldpath(temp_edgemap, pfile)
		--gal.lib.csv.save(temp_edgemap, pfile)
...
--##
	...
	function gal.lib.voronoi.load_vertexmap(pfile)
		...
		if (t_vertexmap == nil) then
			t_vertexmap = gal.lib.csv.load_worldpath("|", pfile)
		end

		if (t_vertexmap ~= nil) then

			for i_p, p_vertexmap in ipairs(t_vertexmap) do

				local e_x, e_z, c_i, n1_i, n2_i = unpack(p_vertexmap)

				local e_pos = {x = tonumber(e_x), z = tonumber(e_z)}

				if not (gal.lib.voronoi.vertexmap[e_pos]) then
					gal.lib.voronoi.vertexmap[e_pos] = {}
				end

				gal.lib.voronoi.vertexmap[e_pos].c_i = tonumber(c_i)
				gal.lib.voronoi.vertexmap[e_pos].n_i = tonumber(n1_i)
				gal.lib.voronoi.vertexmap[e_pos].n_i = tonumber(n2_i)

			end

			minetest.log("[gal.lib.voronoi] Voronoi Vertex Map loaded from file.")

		end
	end

	function gal.lib.voronoi.save_vertexmap(pfile)

		if not pfile or pfile == "" then
			return
		end

		local temp_vertexmap = "#Edge_XPos|Edge_ZPos|Cell_Index|Neighbor1_Index|Neighbor2_Index\n"
		temp_vertexmap = temp_vertexmap .. "#E_X|E_Z|C_I|N1_I|N2_I\n"

		for i_e, i_edge in pairs(gal.lib.voronoi.vertexmap) do

			temp_vertexmap = temp_vertexmap .. i_e.x .. "|" .. i_e.z .. "|" .. i_edge.c_i .. "|" .. i_edge.n1_i .. "|" .. i_edge.n2_i .. "\n"

		end
	
		gal.lib.csv.save_worldpath(temp_vertexmap, pfile)
		--gal.lib.csv.save(temp_edgemap, pfile)
...
--##
	...
	minetest.log("[gal.lib.voronoi]  Voronoi Data Processing ...")
	...
	minetest.log("[gal.lib.voronoi]  Voronoi Data Processing Completed.")

	--minetest.log("[gal.lib.voronoi]  Base Max:" .. base_max .. ";  Base Min:" .. base_min .. ";  Base Range:" .. base_rng .. ";  Ease Factor:" .. easing_factor .. ";")
	--print("[gal.lib.voronoi] Base Max:" .. base_max .. ";  Base Min:" .. base_min .. ";  Base Range:" .. base_rng .. ";  Ease Factor:" .. easing_factor .. ";")

	--minetest.log("[gal.lib.voronoi]  Base Height:" .. base_height)
	--print("[gal.lib.voronoi] Base Height:" .. base_height)
