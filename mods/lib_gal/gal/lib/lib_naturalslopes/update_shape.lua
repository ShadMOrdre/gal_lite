...
local function pick_replacement(slope_type, name, pos, param2)
	local replacement = gal.lib.naturalslopes.get_replacement(name)
...
local function area_pick_replacement(slope_type, data, param2_data, id, index, param2)
	local replacement = gal.lib.naturalslopes.get_replacement_id(id)
...
function gal.lib.naturalslopes.is_free_for_shape_update(pos)
...
function gal.lib.naturalslopes.area_is_free_for_erosion(area, data, index)
...
function gal.lib.naturalslopes.get_replacement_node(pos, node, area, data, param2_data)
	...
	if area then
		is_free = function (at_index) -- always use with new_pos
			return gal.lib.naturalslopes.area_is_free_for_erosion(area, data, at_index)
		end
		...
		is_free = gal.lib.naturalslopes.is_free_for_shape_update
...
function gal.lib.naturalslopes.chance_update_shape(pos, node, factor)
	if factor == nil then factor = 1 end
	local replacement = gal.lib.naturalslopes.get_replacement(node.name)
	if not replacement then return false end
	if (math.random() * (replacement.chance * factor)) < 1.0 then
		return gal.lib.naturalslopes.update_shape(pos, node)
	end
	return false
end
...
function gal.lib.naturalslopes.update_shape(pos, node)
	local replacement = gal.lib.naturalslopes.get_replacement_node(pos, node)
	if replacement then
...
-- at every loop.
function gal.lib.naturalslopes.area_chance_update_shape(minp, maxp, factor, skip, start_i)
	...
	while i <= imax do
		...
		or y == 0 or y == area.zstride - 1 then
			...
			local replacement = gal.lib.naturalslopes.get_replacement_id(data[i])
			if replacement and (math.random() * (replacement.chance * factor)) < 1.0 then
				local new_data = gal.lib.naturalslopes.get_replacement_node(i, data[i], area, data, param2_data)
				if new_data then
					data[i] = new_data.id
...
gal.lib.naturalslopes.progressive_area_updates = {}

function gal.lib.naturalslopes.register_progressive_area_update(minp, maxp, factor, skip)
	table.insert(gal.lib.naturalslopes.progressive_area_updates, {minp = minp, maxp = maxp,
			factor = factor, skip = skip, i = 1})
end

local function progressive_area_update()
	if #gal.lib.naturalslopes.progressive_area_updates == 0 then
		return true
	end
	...
	for area_index, area in ipairs(gal.lib.naturalslopes.progressive_area_updates) do
	...
	local area = gal.lib.naturalslopes.progressive_area_updates[processed_area_index]
	local i = area.i
	...
	while i <= imax do
		...
		gal.lib.naturalslopes.chance_update_shape(pos, node, area.factor)
	...
	table.remove(gal.lib.naturalslopes.progressive_area_updates, processed_area_index)
	return true
end

if gal.lib.naturalslopes.setting_generation_method() == "Progressive" then
	...
	minetest.register_on_shutdown(function()
		if #gal.lib.naturalslopes.progressive_area_updates > 0 then
			...
			for i, area in ipairs(gal.lib.naturalslopes.progressive_area_updates) do
				minetest.log("info", (#gal.lib.naturalslopes.progressive_area_updates - i + 1) .. " remaining area(s)")
				gal.lib.naturalslopes.area_chance_update_shape(area.minp, area.maxp, area.factor, area.skip)
			end
		end
	end)
end
...
function gal.lib.naturalslopes.update_shape_on_walk(player, pos, node, desc, trigger_meta)
	return gal.lib.naturalslopes.get_replacement_node(pos, node)
end
...
minetest.register_chatcommand('updshape', {
	func = function(name, param)
		...
		if gal.lib.naturalslopes.update_shape(node_pos, node) then
...
if gal.lib.naturalslopes.setting_enable_shape_on_generation() then
	if gal.lib.naturalslopes.setting_generation_method() == "Progressive" then
		minetest.register_on_generated(function(minp, maxp, seed)
			gal.lib.naturalslopes.register_progressive_area_update(minp, maxp, gal.lib.naturalslopes.setting_generation_factor(), gal.lib.naturalslopes.setting_generation_skip())
		end)
	else
		minetest.register_on_generated(function(minp, maxp, seed)
			gal.lib.naturalslopes.area_chance_update_shape(minp, maxp, gal.lib.naturalslopes.setting_generation_factor(), gal.lib.naturalslopes.setting_generation_skip())
		end)
	end
end
...
local function on_place_or_dig(pos, force_below)
	local function update(pos, x, y, z, factor)
		...
		gal.lib.naturalslopes.chance_update_shape(new_pos, minetest.get_node(new_pos), factor)
	end
...
if gal.lib.naturalslopes.setting_enable_shape_on_dig_place() then
