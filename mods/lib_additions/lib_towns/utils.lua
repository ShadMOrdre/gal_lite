...
function lib_towns.check_distance_other_lib_towns(center_new_chunk)
	...
	for i, pos in ipairs(lib_towns.settled_areas_in_world) do
		...
		local distance = math.max(math.abs(center_new_chunk.x-pos.x), math.abs(center_new_chunk.z-pos.z))
...
-- evaluate heightmap
--
function lib_towns.evaluate_heightmap(minp, maxp, pgen)

	local h_diff
	--if pgen and pgen == "on_gen" then

	--	h_diff = gal.mapgen.chunk_max_altitude - gal.mapgen.chunk_min_altitude
	...
	if lib_towns.debug == true then
		...
		minetest.chat_send_all("heightdiff ".. h_diff)
	end
