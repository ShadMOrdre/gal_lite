...
-- ## VORONOI DISTANCE CALCULATIONS
				...
				local t_dist = gal.lib.metrics.get_distance({x = (t_x - x), z = (t_z - z)}, dist_metric)
				local tcontinental = t_dist * v_pscale
--
				--set master, parent, cell y values (elevation)
				local m2t_y = (gal.lib.metrics.get_distance({x = (t_x - m_x), z = (t_z - m_z)}, dist_metric) * v_cscale)
				local p2m_y = (gal.lib.metrics.get_distance({x = (m_x - p_x), z = (m_z - p_z)}, dist_metric) * v_pscale)
				local c2p_y = (gal.lib.metrics.get_distance({x = (p_x - c_x), z = (p_z - c_z)}, dist_metric) * v_mscale)
				
				local m_y = t_y - m2t_y
				local p_y = m_y - p2m_y
				local c_y = p_y - c2p_y

				--local m2t_vslope, m2t_vrise, m2t_vrun, m2t_hslope, m2t_hrise, m2t_hrun = gal.lib.metrics.get_3d_slope({x = t_x, y = t_y, z = t_z}, {x = m_x, y = m_y, z = m_z})
				--local p2m_vslope, p2m_vrise, p2m_vrun, p2m_hslope, p2m_hrise, p2m_hrun = gal.lib.metrics.get_3d_slope({x = m_x, y = m_y, z = m_z}, {x = p_x, y = p_y, z = p_z})
				--local c2p_vslope, c2p_vrise, c2p_vrun, c2p_hslope, c2p_hrise, c2p_hrun = gal.lib.metrics.get_3d_slope({x = p_x, y = p_y, z = p_z}, {x = c_x, y = c_y, z = c_z})
				----local sl_terrain = (-m2t_vslope * m2t_vrun) + (-p2m_vslope * p2m_vrun) + (-c2p_vslope * c2p_vrun)
				----local sl_terrain = (m2t_vrise / -m2t_vslope) + (p2m_vrise / -p2m_vslope) + (c2p_vrise / -c2p_vslope)

				local sl_mterrain = gal.lib.metrics.get_trilinear_interpolation({x = t_x, y = t_y, z = t_z}, {x = m_x, y = m_y, z = m_z})
				local sl_pterrain = gal.lib.metrics.get_trilinear_interpolation({x = m_x, y = m_y, z = m_z}, {x = p_x, y = p_y, z = p_z})
				local sl_cterrain = gal.lib.metrics.get_trilinear_interpolation({x = p_x, y = p_y, z = p_z}, {x = c_x, y = c_y, z = c_z})

				----local sl_terrain = sl_mterrain + sl_pterrain + sl_cterrain) / 3
				--local sl_terrain = t_y - ((sl_mterrain + sl_pterrain + sl_cterrain) / 3)
				local sl_terrain = sl_cterrain
--
--[[
				------get distance measurements for cells
				local c2p_dist = gal.lib.metrics.get_distance({x = (p_x - c_x), z = (p_z - c_z)}, dist_metric)
				----local c2m_dist = gal.lib.metrics.get_distance({x = (m_x - c_x), z = (m_z - c_z)}, dist_metric)
				local c2t_dist = gal.lib.metrics.get_distance({x = (t_x - c_x), z = (t_z - c_z)}, dist_metric)
				local p2m_dist = gal.lib.metrics.get_distance({x = (m_x - p_x), z = (m_z - p_z)}, dist_metric)
				--local p2t_dist = gal.lib.metrics.get_distance({x = (t_x - p_x), z = (t_z - p_z)}, dist_metric)
				local m2t_dist = gal.lib.metrics.get_distance({x = (t_x - m_x), z = (t_z - m_z)}, dist_metric)

				local b_cpm2t_dist = c2p_dist + p2m_dist + m2t_dist
				local b_cp2m_dist = c2p_dist + p2m_dist
				local b_pm2t_dist = p2m_dist + m2t_dist

				----current voronoi distance
				local n_dist = c_dist + b_cpm2t_dist
				--local n2p_dist = c_dist + c2p_dist
				--local n2m_dist = c_dist + c2p_dist + p2m_dist
				--local n2t_dist = c_dist + c2p_dist + p2m_dist + m2t_dist

				local n_y = t_y

				if t_dist < m_dist then
					n_y = t_y - tcontinental
				else
					if t_dist < p_dist then
						n_y = m_y - mcontinental
					else
						if t_dist < c_dist then
							n_y = p_y - pcontinental
						else
							n_y = c_y - ccontinental
						end
					end
				end
--]]
...
--## BIOMES GENERATION
				...
				local nhumid = gal.mapgen.get_humid_scalar(z) + (nbuf_humiditymap[z-minp.z+1][x-minp.x+1] + nbuf_humidityblend[z-minp.z+1][x-minp.x+1]) + (((x - t_x) * (0.01 / mg_world_scale)) + (math.sin((x - t_x) * (0.001 / mg_world_scale))))
