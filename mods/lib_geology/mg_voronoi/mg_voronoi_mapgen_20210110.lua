...
--##	(2400 * 0.1) * 0.25 == 240 / 4 == 60
	...
	mg_voronoi.mg_noise_3d_density = 13 * mg_world_scale
		...
		for z = minp.z, maxp.z do
			for y = minp.y, maxp.y do
				for x = minp.x, maxp.x do
					...
					----local n_y = minetest.get_perlin(np_terrain):get_2d({x=x,y=z})
					--local n_y = nbuf_terrain[z-minp.z+1][x-minp.x+1]
					--local n_y = nbuf_terrain[z-minp.z+1][x-minp.x+1] * mg_world_scale_inverse
					--local n_y = gal.mapgen.heightmap[index2d]
					local n_y = mg_voronoi.heightmap2d[index2d] * 0.1
