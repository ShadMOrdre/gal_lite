...
-- ## VORONOI DISTANCE CALCULATIONS
				...
				local xm_slope = xm_rise / xm_run
				local xp_slope = xp_rise / xp_run
				local xc_slope = xc_rise / xc_run
				...
				local xpcontinental = (xp_dist * v_pscale) - mp_diff
				--local xccontinental = (xc_dist * v_mscale) - xc_slope
				local xccontinental = (xc_dist * v_mscale) - pc_diff

				local xbcontinental = (xmcontinental + xpcontinental + xccontinental) - x_diff
...
--]]
				...
				local xbterrain = (mg_base_height - xbcontinental) + (mg_base_height * 0.4)
				--local xaheight = (aterrain * math.sin(nfill)) * (1 / bcontinental)
				--local xbheight = (xbterrain + aterrain) * (1 / bcontinental)
				--local belevation = (xbheight + xaheight) * (mg_world_scale / 0.01)
				local belevation = xbterrain + aterrain


					----local fterrain = c_y - (ccontinental + (x_diff * mg_world_scale))
					--local fterrain = c_y - (ccontinental + x_diff)
